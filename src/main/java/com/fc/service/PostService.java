...
public class PostService {
    ...
    public int publishPost(Post post) {
        //构造帖子
        // MyUtil.formatDate 转换时间格式
        post.setPublishTime(MyUtil.formatDate(new Date()));//设置发布时间
        post.setReplyTime(MyUtil.formatDate(new Date()));//设置回复时间
        //初始化回复数 点赞数 浏览数
    ...
    public PageBean<Post> listPostByTime(int curPage) {
        ...
        for(Post post : postList){//实时更新点赞数
            post.setLikeCount((int)(long)jedis.scard(post.getPid()+":like"));
        }

        //构造PageBean
        // allPage 所有页数 curPage 当前所在页数
    ...
    //取消点赞
    public String clickUnlike(int pid, int sessionUid){
        Jedis jedis = jedisPool.getResource();
        //从集合中移除uid
        jedis.srem(pid+":like", String.valueOf(sessionUid));
        //减少用户获赞数
        // Hincrby 命令用于为哈希表中的字段值加上指定增量值。
        jedis.hincrBy("vote",sessionUid+"",-1);
        ...
        postMapper.reduceLikeCount(pid);
