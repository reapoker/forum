...
public class PostController {
    ...
    //删帖
    @RequestMapping(value = "/deletePost.do",produces = "text/plain;charset=UTF-8")
    public @ResponseBody
    boolean deletePost(int pid){
        postService.deletePost(pid);
        System.out.println("删除帖子"+pid);
        return true;
    }
    ...
    public String listPostByTime(int curPage,Model model){
        ...
        List<User> userList = userService.listUserByTime();//用户列表
        List<User> hotUserList = userService.listUserByHot();//热帖
...
//        model.addAttribute("liked",true);
    ...
    //取消点赞
    @RequestMapping(value = "/ajaxClickUnlike.do",produces = "text/plain;charset=UTF-8")
    public @ResponseBody
    String ajaxClickUnlike(int pid, HttpSession session){
        int sessionUid = (int) session.getAttribute("uid");
//        model.addAttribute("liked",false);
        return postService.clickUnlike(pid,sessionUid);
    }
}
