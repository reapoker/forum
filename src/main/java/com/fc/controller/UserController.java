...
public class UserController {
    ...
    public String toMyProfile(HttpSession session,Model model) {
        ...
        //获得好友列表
        List<User> friend = userService.listUserByFriend(sessionUid);
    ...
    public String toProfile(int uid,Model model,HttpSession session) {
        ...
        //判断是否关注当前用户
        int following = userService.getFollowStatus(sessionUid,uid);
        //获得用户信息
        User user = userService.getProfile(sessionUid, uid);
        //获得发帖列表
        List<Post> postList =  postService.getPostList(uid);
        //获得好友列表
        List<User> friend = userService.listUserByFriend(uid);
        //向模型中添加数据
    ...
    public String followed(int uid, String name, Model model, HttpSession session){
        int sessionUid = (int) session.getAttribute("uid");
        userService.followed(sessionUid,uid);

        model.addAttribute("title","添加好友成功");
        model.addAttribute("info","您和"+name+"已成为好友");

        return "prompt/promptInfo";
    }

    @RequestMapping("/refuse.do")
    public String refuse(int uid, Model model, HttpSession session){
        int sessionUid = (int) session.getAttribute("uid");
        userService.refused(sessionUid,uid);

        model.addAttribute("title","已拒绝");
        model.addAttribute("info","您已拒绝该好友申请！");
    ...
    public String verify(String code){
        String password = code.substring(0,8);
        userService.verifyForgetPassword(code,password);
    ...
    public List<User> selectLikeUser(String likename){
        List<User>likeUesrs = new LinkedList<>();
        if (likename.equals("")) likename = null;
        if (likename != null)
            likeUesrs = userService.selectLikeUser(likename);
        return likeUesrs;
    }
}
